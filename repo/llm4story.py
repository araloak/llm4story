...
def ask_why(story,new_info=[None],plots=None,depth=1,width=2,unclarities=None, queries = {}):
    ...
    for index,unclarity in enumerate(plots):
        prompt_1 = 'Here is a story: \n' + story +'\n' + 'An unclarity is: \n'+ unclarity +'\n' +\
                   'Other than coincidence and unknown factors, please point out '+str(width)+\
                   ' major explanations to rationalize the story. The explanation should be concise and echo the topic: '+ \
...
def add_new_info(story,picked_info):
    ...
    # reasons = "\n".join(reasons)
    prompt = 'Here is a story: \n' + story +'\n' +'Here are a list of questions and explanations : \n'+ picked_info + '\n'+ \
             'Please add the explanatory information into the story coherently to address the questions and preserve other details by repeating the unchanged parts and only modifying the necessary paragraphs. As the story is too long, to make sure all the details are well covered, only describe the first half of the story and only tell the second half when I request.'
